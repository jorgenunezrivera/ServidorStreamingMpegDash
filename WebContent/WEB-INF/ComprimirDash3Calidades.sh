...
#Comprobar directorio
    ...
    /usr/bin/ffmpeg -y -i $fname.$fextension -c:a aac -ac 2 -ab 128k -c:v libx264 -x264opts 'keyint=24:min-keyint=24:no-scenecut' -b:v 1500k -maxrate 1500k -bufsize 1000k -vf "scale=-2:720" "$fname"1500.$oextension

    /usr/bin/ffmpeg -y -i $fname.$fextension -c:a aac -ac 2 -ab 128k -c:v libx264 -x264opts 'keyint=24:min-keyint=24:no-scenecut' -b:v 800k -maxrate 800k -bufsize 500k -vf "scale=-2:540" "$fname"800.$oextension
